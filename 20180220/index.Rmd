...
date: 'Slides: https://webinars.cpsievert.me <br> <br>  `r icon::fa_twitter()` <a href="https://twitter.com/cpsievert">@cpsievert</a> <br> `r icon::fa_github()` <a href="https://github.com/cpsievert">@cpsievert</a> <br> `r icon::fa_envelope()` <cpsievert1@gmail.com> <br /> `r icon::fa_globe()` <https://cpsievert.me/> <br> <br> Slides released under <a href="https://github.com/cpsievert/talks/blob/gh-pages/LICENSE">Creative Commons</a>'
---
...
### Make your ggplots interactive!

```r
p <- ggplot(diamonds, aes(price, stat(density), colour = cut)) +
  geom_freqpoly(binwidth = 500)
ggplotly(p, tooltip = "density")
```

<img src="ggplotly.gif" width="100%" >
...
.pull-right[
<img src="shiny-plotly-y.png" width="100%" >
]
...
### Some outputs can be treated like an input!
...
1. Full draw performed only once (on page load).
2. Every 100 milliseconds, re-run the `observe()` expression.
3. Invoke a plotly.js [restyle](https://plot.ly/javascript/plotlyjs-function-reference/#plotlyrestyle) on the (`bars`) output with three new random values for the bar heights.
...
* I'm using **ggstat** because `bin_fixed()` is quite fast and simple.
  ...
  * [Binning with **data.table**](https://www.r-bloggers.com/using-data-table-for-binning/) comparable, but slower and complicated.
...
* Concepts covered here and the (upcoming) plotly book can be reused in *many* ways
...
#### `event_data()` now supports more plotly.js events, including 'continuous' ones  like `"plotly_brushing"`
  ...
  * Currently available on GitHub `devtools::install_github("ropensci/plotly")`
...
<img src="tom-cruise.gif" width="100%" >

* I'm still learning about best practices and patterns for linking large data visualizations responsively. 
* Please do share your experiences and let me know where the rough edges are.
